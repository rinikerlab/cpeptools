...
#
#     def __init__(self, smiles):
#         self.fragment = Chem.MolFromSmiles(smiles)
#         # self.fragment(Chem.MolFromPDBChem.MolToPDBBlock(self.fragment))
#
#         for idx, atm in enumerate(self.fragment.GetAtoms()):
#             name = "{}{}".format(atm.GetSymbol(), idx)
#             atm.SetMonomerInfo(Chem.AtomPDBResidueInfo())
#             atm.GetPDBResidueInfo().SetName(" {: <3s}".format(name))
#             atm.GetPDBResidueInfo().SetIsHeteroAtom(False)
...
#
#     def set_resname(self, resname):
#         for atm in self.fragment.GetAtoms():
#             atm.GetPDBResidueInfo().SetResidueName(resname)
#
#     def set_resnum(self, resnum):
#         for atm in self.fragment.GetAtoms():
...
#
#     def rename(self, oldname, newname):
#         name_pool = [atm.GetPDBResidueInfo().GetName() for atm in self.fragment.GetAtoms()]
#
#         if newname.strip() in name_pool:
#             raise ValueError("{} has already been used in the fragment".format(newname))
#
...
#         from ..mol_ops import draw_mol_with_property
#         if property is None:
#             draw_mol_with_property(self.fragment, {idx : atm.GetPDBResidueInfo().GetName() for idx, atm in enumerate(self.fragment.GetAtoms())})
#         else:
#             draw_mol_with_property(self.fragment, property)
#########################################
#########################################
#########################################
#########################################
...
#         from ..mol_ops import draw_mol_with_property
#         if property is None:
#             draw_mol_with_property(self.peptide, {idx : atm.GetPDBResidueInfo().GetName() for idx, atm in enumerate(self.peptide.GetAtoms())})
#         else:
#             draw_mol_with_property(self.peptide, property)
#
#
# class CycloPeptide(Peptide):
#     """
